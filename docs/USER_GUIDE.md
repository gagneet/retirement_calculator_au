# Australian Retirement Calculator - User Guide

Welcome to the Enhanced Australian Retirement Calculator! This guide will walk you through how to use the tool to gain valuable insights into your retirement planning.

## Table of Contents
1. [Getting Started](#1-getting-started)
    - [New Users: Onboarding](#new-users-guided-onboarding)
    - [Returning Users: Uploading Data](#returning-users-uploading-data)
2. [The Main Dashboard](#2-the-main-dashboard)
    - [Input Sections](#input-sections)
    - [Running a Calculation](#running-a-calculation)
3. [Understanding Your Results](#3-understanding-your-results)
    - [Comprehensive Results Dashboard](#comprehensive-results-dashboard)
    - [Persona Insights](#persona-insights)
    - [Contextual Alerts & Guidance](#contextual-alerts--guidance)
    - [Health Check Dashboard](#health-check-dashboard)
4. [Diving Deeper: Analysis Tabs](#4-diving-deeper-analysis-tabs)
    - [Charts & Visualizations](#charts--visualizations)
    - [Actionable Risk Analysis](#actionable-risk-analysis)
    - [Scenario Comparison Matrix](#scenario-comparison-matrix)
5. [Your Personalized Action Plan](#5-your-personalized-action-plan)
6. [Advanced Features](#6-advanced-features)
    - [Monte Carlo Simulation](#monte-carlo-simulation)
    - [Stress Testing](#stress-testing)
7. [Data Management](#7-data-management)
    - [Exporting Your Plan](#exporting-your-plan)
    - [Data Security](#data-security)
8. [Troubleshooting](#8-troubleshooting)

---

## 1. Getting Started

When you first open the calculator, you are greeted with a welcome page.

`[Screenshot of the main welcome page with "New User" and "Returning User" buttons]`

### New Users: Guided Onboarding

If you're new, click the large green **"New User? Start Here"** button. This will launch a step-by-step guided onboarding process to collect all the necessary information.

The onboarding process is divided into five tabs:
1.  **Household:** Your age, partner's age, and retirement goals.
2.  **Finances:** Your income, superannuation balance, savings, and investments.
3.  **Property:** Details about your home and any investment properties.
4.  **Goals:** Your desired retirement lifestyle and risk tolerance.
5.  **Review:** A summary of all your entered information for a final check.

Complete each tab in order. Once you finish the review, the calculator will automatically generate your first comprehensive retirement plan.

### Returning Users: Uploading Data

If you have used the calculator before and saved your data, click the **"Returning User? Upload Data"** button. You can upload a previously exported JSON file to instantly populate the calculator with your information.

## 2. The Main Dashboard

After onboarding, you'll have access to the main dashboard, where you can fine-tune your inputs.

### Input Sections
The left-hand side of the dashboard contains all your financial details. You can update any of these values at any time. The key sections include:
-   **Personal Details:** Ages, retirement timeline, and lifespan expectations.
-   **Financial Details:** Salaries, super balances, savings, and investments.
-   **Property Details:** Your home and investment property information.
-   **Risk Profile:** Your tolerance for investment risk and other risk-related factors.
-   **Economic Assumptions:** Advanced settings for inflation, investment returns, etc. (It's best to leave these as defaults unless you are an experienced user).

### Running a Calculation
Whenever you change an input, you can click the **"Calculate Enhanced Projection"** button to update your results. The dashboard will update instantly with new insights.

## 3. Understanding Your Results

After running a calculation, the results are displayed in a series of easy-to-understand dashboards.

### Comprehensive Results Dashboard
This is the first thing you'll see. It provides a high-level overview of your retirement readiness.

`[Screenshot of the Comprehensive Results Dashboard]`

-   **Total Assets:** Your projected total assets at retirement.
-   **Years Left:** The number of years until your planned retirement.
-   **Success Rate:** The probability that your money will last through retirement, based on a Monte Carlo simulation.
-   **Readiness Score:** An overall score (out of 100) that summarizes your retirement preparedness.

### Persona Insights
Based on your inputs, the calculator assigns you a **Retirement Persona**.

`[Screenshot of the Persona Insights section, e.g., "High Earner"]`

This helps you understand your financial situation in a relatable way and provides key strategies tailored to your profile (e.g., "Tax-Effective Wealth Building" for a High Earner).

### Contextual Alerts & Guidance
The calculator's intelligence system provides real-time alerts and guidance.
-   **Alerts (⚠️):** These highlight potential issues or risks in your plan, such as a low savings rate or high debt.
-   **Guidance (💡):** These are smart tips that suggest what you could do next, like running a Monte Carlo simulation or filling out more details to improve the accuracy of your plan.

### Health Check Dashboard
This dashboard gives you a quick visual check on key areas of your financial health.

`[Screenshot of the Health Check Dashboard with green, yellow, red indicators]`

Metrics include:
-   **Savings Rate:** Are you saving enough?
-   **Pension Optimization:** Are you on track to make the most of the Age Pension?
-   **Asset Allocation:** Is your investment mix appropriate for your age and risk profile?
-   **Tax Efficiency:** Are you making the most of tax-advantaged strategies?

## 4. Diving Deeper: Analysis Tabs

For more detailed analysis, you can explore the various tabs at the top of the results section.

### Charts & Visualizations
This tab provides graphical representations of your retirement plan.
-   **Income Breakdown Chart:** A stacked area chart showing where your income will come from in retirement (e.g., Super Drawdown, Age Pension, Investment Income).
-   **Monte Carlo Fan Chart:** Shows the range of possible outcomes for your portfolio, illustrating the best-case, worst-case, and most likely scenarios.

### Actionable Risk Analysis
This powerful tool shows you the top 3 actions you can take to improve your plan's success rate.

`[Screenshot of the Actionable Risk Analysis with top 3 improvements]`

For each suggestion, it shows:
-   The potential improvement to your success rate.
-   The cost and benefit of the action.

### Scenario Comparison Matrix
This feature allows you to compare different "what-if" scenarios side-by-side. For example, you can see how retiring two years earlier or increasing your savings rate would affect your final outcome.

## 5. Your Personalized Action Plan

After analyzing your data, the system generates a timeline-based action plan.

`[Screenshot of the Final Action Plan]`

This plan is broken down into:
-   **Phase 1: Immediate Actions (Next 30 Days):** Quick wins that have a high impact.
-   **Phase 2: Short-Term Strategy (1-6 Months):** Steps to build momentum.
-   **Phase 3: Medium-Term Development (6-12 Months):** Actions to establish good financial habits.
-   **Phase 4: Long-Term Optimization (12+ Months):** Advanced strategies for the future.

## 6. Advanced Features

### Monte Carlo Simulation
You can run a detailed Monte Carlo simulation by clicking the **"Run Enhanced Monte Carlo"** button in the "Advanced Calculator" section. This simulates thousands of possible market conditions to test the robustness of your plan.

### Stress Testing
The **"Run Stress Test Scenarios"** feature subjects your plan to historical financial crises, like the Global Financial Crisis or a major property market correction, to see how it holds up.

## 7. Data Management

### Exporting Your Plan
You can export your results at any time using the export button. Options include:
-   **PDF:** A comprehensive, easy-to-read report.
-   **XLSX (Excel):** A detailed spreadsheet with all your data and charts.
-   **CSV:** Raw data for use in other tools.
-   **JSON:** A complete backup of your inputs that you can re-upload later.

### Data Security
Your financial information is safe. All data is stored and processed **only in your browser**. Nothing is sent to any external servers.

## 8. Troubleshooting

-   **Charts not loading?** Try refreshing the page.
-   **Data not saving?** Ensure your browser has localStorage enabled.
-   **Export not working?** Try disabling any pop-up blockers or using a different browser.

For any other issues, please refer to the `DEVELOPER_GUIDE.md` for more technical troubleshooting steps.